...
type ValkeyUserDataSourceModel struct {
...
type ValkeyUserDataSource struct{ client *client.Client }

func NewValkeyUserDataSource() datasource.DataSource { return &ValkeyUserDataSource{} }

func (d *ValkeyUserDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_valkey_user"
}

func (d *ValkeyUserDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	specAttrs := map[string]schema.Attribute{
		...
		"key_patterns": schema.ListAttribute{Computed: true, ElementType: types.StringType},
		"password":     schema.StringAttribute{Computed: true, Sensitive: true},
		"valkey_id":    schema.StringAttribute{Computed: true},
	}
...
func (d *ValkeyUserDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ValkeyUserDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var state ValkeyUserDataSourceModel
	...
	if idSet {
		apiData, err = d.client.Get(ctx, "/api/v1/valkey-user", state.ID.ValueString())
	} else {
		apiData, err = d.client.GetByName(ctx, "/api/v1/valkey-user", state.Name.ValueString())
	}
	if err != nil {
	...
	state.Spec = &ValkeyUserSpecModel{}
